    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="w-72 sm:w-80 md:w-96 h-[460px] sm:h-[500px] 
                     bg-gradient-to-b from-gray-900 via-black to-gray-950
                     text-white rounded-xl shadow-2xl flex flex-col border border-gray-800"
        >
          {/* Header */}
          ...
          {/* Options */}
          <div className="p-2 border-t border-gray-800 flex flex-wrap gap-2 bg-black/50 rounded-b-xl">
            {options.map((opt, i) => (
              <button
                key={i}
                onClick={() => handleOptionClick(opt)}
                className="px-2 py-1.5 bg-gradient-to-r from-white to-gray-200 
                           text-black text-[11px] sm:text-xs rounded-lg font-medium 
                           shadow-sm hover:scale-105 hover:shadow-gray-400/30 
